...
func redrawAll() {
	...
	midy := h / 2
	midx := (w - editBox_width) / 2

	// unicode box drawing chars around the edit box
	termbox.SetCell(midx-1, midy, '│', coldef, coldef)
	termbox.SetCell(midx+editBox_width, midy, '│', coldef, coldef)
	termbox.SetCell(midx-1, midy-1, '┌', coldef, coldef)
	termbox.SetCell(midx-1, midy+1, '└', coldef, coldef)
	termbox.SetCell(midx+editBox_width, midy-1, '┐', coldef, coldef)
	termbox.SetCell(midx+editBox_width, midy+1, '┘', coldef, coldef)
	fill(midx, midy-1, editBox_width, 1, termbox.Cell{Ch: '─'})
	fill(midx, midy+1, editBox_width, 1, termbox.Cell{Ch: '─'})

	editBox.Draw(midx, midy, editBox_width, 1)
